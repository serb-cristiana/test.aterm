<?php
session_start();
$_SESSION['pag'] = 'dealeri';
include 'functii.php';
VerificareOcolireLogin();
if (VedeDealeri() == FALSE) {die('Forbiden acces!');};
ConectareBd();
?>
...
$date = $conbd->query("SELECT d.ID AS ID, d.Nume AS Nume, d.Reprezentant AS Reprezentant, "
  . "d.Adresa AS Adresa, d.TelMobil AS TelMobil, "
  . "d.CNP AS CNP, d.CUI AS CUI, d.RC AS RC, d.email AS email, "
  . "d.Banca AS Banca, d.Cont AS Cont, d.CodPartener AS CodPartener, d.Activ AS Activ, d.IDLivrare AS IDLivrare, "
      ...
      while ($row=$date->fetch_assoc()){?>
        <tr class="<?php if ($row['Activ'] == 0){echo 'danger';};?>">
          ...
          <!------------------- butonul de modificare si forma modala care apare la apasarea lui -------------------------->
            <a class="btn btn-default btn-xs <?php if(ModificaDealeri() == FALSE){echo ' hidden';}?>" 
              title="Editeaza dealer"      
              data-toggle="modal" data-target="#modificareDealer<?php echo $row['ID']; ?>">
              <span class="glyphicon glyphicon-edit" aria-hidden="true"></span>
            </a>
          <!-- Modal -->
          <div class="modal fade" 
              id="modificareDealer<?php echo $row['ID']; ?>" 
            ...
            <div class="modal-dialog modal-lg" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  ...
                  <h4 class="modal-title" id="myModalLabel">Editare partener <?php echo $row['Nume']; ?></h4>
                ...
                <form class="form-horizontal">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-3">
                        <input 
                          type="text" 
                          class="form-control" 
                          id="Reprezentant<?php echo $row['ID']; ?>"
                          value="<?php echo $row['Reprezentant']; ?>" />
                      </div>
                      <label for="CNP" class="col-sm-3 control-label">CNP reprezentant:</label>
                      <div class="col-sm-3">
                        <input 
                          type="text" 
                          class="form-control" 
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-3">
                        <input 
                          type="text" 
                          class="form-control" 
                          id="CodPartener<?php echo $row['ID']; ?>"
                          value="<?php echo $row['CodPartener']; ?>" />
                      </div>
                    </div>  
                  ...
                  <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" onclick="updateData('updateDealer', <?php echo $row['ID']; ?>)">Salveaza</button>
                  </div>
                </form>                
              </div>
            </div>
